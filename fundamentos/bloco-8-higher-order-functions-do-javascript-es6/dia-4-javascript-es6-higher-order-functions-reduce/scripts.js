...
const sumEven = numbers.reduce(((accumulator, number) => 
  (number % 2 === 0) ? accumulator + number : accumulator), 0);
...
const melhorMateria = estudantes.map((estudante) => {
  const melhorNota = estudante.materias.reduce(((accNota, nota) => {
    return (accNota > nota) ? accNota : nota
  }),0)
  return {'name': estudante.nome,'materia': melhorNota.name }
})
